...
public class CtestController {
        ...
        @Operation(summary = "Get all Tests Results", description = "Get all Covid-19 Test results by their credentials", security = @SecurityRequirement(name = "bearerAuth"), tags = {
                        "COVID-19 Test" })
        @ApiResponses({ @ApiResponse(responseCode = "200", description = "Successful retrieval", content = @Content(mediaType = "application/json", array = @ArraySchema(schema = @Schema(implementation = Ctest.class)))) })
        ...
        @Operation(summary = "Add new Test result", description = "Add new Covid-19 Test result by credentials", security = @SecurityRequirement(name = "bearerAuth"), tags = {
                        "COVID-19 Test" })
        @ApiResponses({ @ApiResponse(responseCode = "201", description = "Successful created new Covid-19 Test result", content = @Content(mediaType = "application/json", schema = @Schema(implementation = Ctest.class))), })
        ...
        @Operation(summary = "Update Test result", description = "Update Covid-19 Test result by credentials and provided ID", security = @SecurityRequirement(name = "bearerAuth"), tags = {
                        "COVID-19 Test" })
        @ApiResponses({ @ApiResponse(responseCode = "200", description = "Successful updated  Covid-19 Test result", content = @Content(mediaType = "application/json", schema = @Schema(implementation = Ctest.class))), })
        ...
        @Operation(summary = "Delete Test result", description = "Delete Covid-19 Test result by credentials and provided ID", security = @SecurityRequirement(name = "bearerAuth"), tags = {
                        "COVID-19 Test" })
        @ApiResponses({ @ApiResponse(responseCode = "204", description = "Successful deleted Covid-19 Test result", content = @Content) })
        ...
        /**
         * Get the next Covid test date for the employee based on the restraunts test
         * frequency and the employees latest covid test date Username and not
         * principal.getName() is used as we want business owners to be able to get this
         * informaiton as well "/employee/{username}/ctests/{ctestId}" If there is no
         * user with the given "username" throw a UserNotFoundException If there is no
         * employee with the user, throw a EmployeeNotFoundException If there is no
         * Ctests with the given "id", throw a CtestNotFoundException if Ctests is empty
         * it would return todays date
         * 
         * @param username username of employee
         */
